<template>
  <div class="item">
    <div class="content-row">
      <label class="content">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer laoreet orci non arcu scelerisque, at fringilla risus fermentum. Vestibulum et nisl tempus, tempor risus rutrum, sollicitudin metus. Vivamus sagittis eros magna, eu gravida sem faucibus quis. Proin condimentum aliquet nisl, eget tempor ex. Vivamus sit amet felis varius, porta lorem vitae, posuere ex. Morbi in lacinia sapien. Mauris finibus porta ante vel sodales.
        Pellentesque pretium, felis vel iac</label>
      <img src="../public/5e5c899db263d713b24720a6cc1a71fa.jpg"/>
    </div>
    <div class="content-row">
      <img src="../public/My-project-2.png">
      <label class="content">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer laoreet orci non arcu scelerisque, at fringilla risus fermentum. Vestibulum et nisl tempus, tempor risus rutrum, sollicitudin metus. Vivamus sagittis eros magna, eu gravida sem faucibus quis. Proin condimentum aliquet nisl, eget tempor ex. Vivamus sit amet felis varius, porta lorem vitae, posuere ex. Morbi in lacinia sapien. Mauris finibus porta ante vel sodales.
        Pellentesque pretium, felis vel iac</label>
      <!--  <iframe width="800" height="640" src="https://www.youtube.com/embed/UeCdBVHYa_8" title="I Made a Bad Decision – Framework Investment Update" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>-->
    </div>
    <div class="content-row">
      <label class="content">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer laoreet orci non arcu scelerisque, at fringilla risus fermentum. Vestibulum et nisl tempus, tempor risus rutrum, sollicitudin metus. Vivamus sagittis eros magna, eu gravida sem faucibus quis. Proin condimentum aliquet nisl, eget tempor ex. Vivamus sit amet felis varius, porta lorem vitae, posuere ex. Morbi in lacinia sapien. Mauris finibus porta ante vel sodales.
        Pellentesque pretium, felis vel iac</label>
      <img src="../public/Social-9.jpg"/>
    </div>
  </div>
</template>

<script>
export default {
  name: "App"
}
</script>

<style scoped>

</style>